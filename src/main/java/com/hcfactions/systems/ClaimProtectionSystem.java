...
/**
 * Handles block place protection based on claim ownership.
 *
 * Rules:
 * - Faction claim -> BLOCKED (protected territory, unless admin)
 * - Own guild claim -> ALLOWED
 * - Same faction, different guild -> BLOCKED
 * - Enemy faction claim -> CANNOT BUILD
 * - Unclaimed land -> ALLOWED for everyone
 */
public class ClaimProtectionSystem extends EntityEventSystem<EntityStore, PlaceBlockEvent> {

    private static final Message MSG_CANNOT_BUILD = Message.raw("You cannot build on this guild's land!").color(Color.RED);
    ...
    public ClaimProtectionSystem(HC_FactionsPlugin plugin) {
    ...
    public void handle(int index, @NonNullDecl ArchetypeChunk<EntityStore> archetypeChunk,
        ...
        if (claim == null) {
            // Unclaimed land - everyone can build
            return;
        }
        ...
        // Solo player claims - only owner can build
        ...
        if (playerFactionId != null && playerFactionId.equals(claim.getFactionId())) {
            ...
            if (plugin.getConfig().isProtectionSameFactionGuildAccess()) {
                return; // Same faction can build in allied guild claims
            }
            event.setCancelled(true);
            playerRef.sendMessage(MSG_CANNOT_BUILD);
            return;
        ...
        // Enemy faction - check if they can build
        if (plugin.getConfig().isProtectionEnemyCanBuild()) {
            return; // Enemies can build in enemy territory
        }
        event.setCancelled(true);
        playerRef.sendMessage(MSG_CANNOT_BUILD);
    }
